...
async function main() {
    ...
    try {
        ...
        const harvestMetadata = await createMetadataHash(
            productBatch,
            1,
            "Harvested",
            {
        ...
        const qualityMetadata = await createMetadataHash(
            productBatch,
            1,
            "Quality Tested",
            {
        ...
        const farmer2Location = await getStakeholderLocation(
            stakeholderManager,
            farmer2.address
        )
        const appleMetadata = await createMetadataHash(
            productBatch,
            2,
            "Harvested",
            {
        ...
        const saleMetadata = await createMetadataHash(
            productBatch,
            1,
            "Sold to Processor",
            {
        ...
        const pickupMetadata = await createMetadataHash(
            productBatch,
            1,
        ...
        console.log(
            "   📍 Shipper1 picked up from farmer (provenance recorded)"
        )
        ...
        const portMetadata = await createMetadataHash(
            productBatch,
            1,
        ...
        await shipmentTracker
            .connect(shipper1)
            .updateLocation(1, "Highway 101 - En route to Processing")
        const transitMetadata = await createMetadataHash(
            productBatch,
            1,
            "In Transit - Highway",
            {
        ...
        console.log(
            "   📍 En route to processing facility (provenance recorded)"
        )
        ...
        const deliveryMetadata = await createMetadataHash(
            productBatch,
            1,
        ...
        const processingMetadata = await createMetadataHash(
            productBatch,
            1,
        ...
        const qcMetadata = await createMetadataHash(
            productBatch,
            1,
        ...
        const packagingMetadata = await createMetadataHash(
            productBatch,
            1,
        ...
        console.log(
            " Mangoes processed into juice with complete provenance tracking"
        )
        ...
        const provenanceSummary = await provenanceTracker.getProvenanceSummary(
            1
        )
        console.log(
            `    First Action: ${provenanceSummary[1]} at ${provenanceSummary[2]}`
        )
        console.log(
            `   📅 Started: ${new Date(
                Number(provenanceSummary[3]) * 1000
            ).toLocaleDateString()}`
        )
        console.log(
            `   📍 Last Action: ${provenanceSummary[5]} at ${provenanceSummary[6]}`
        )
